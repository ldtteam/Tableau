...
There are two more types of relationships: `requiredDependency` and `optionalDependency`, however these are not manually configurable here.
These are provided through the [mod dependencies](managing-dependencies) block.
