...
public class NeoGradleProjectPlugin implements Plugin<Project> {
	...
	private void configureRuns(@NotNull Project target) {
		...
		runManager.addAllLater(
				extension.getDataGenerationRuns().map(names -> names.stream()
						.map(name -> createDataRun(target, runManager, projectExtension, extension, name, sourceSetExtension.maybeCreate(SourceSet.MAIN_SOURCE_SET_NAME)))
						.toList())
		);
	...
	private static Run createDataRun(
			final @NotNull Project target,
            final RunManager runManager,
            final ProjectExtension projectExtension,
			final NeoGradleExtension extension,
			final String runName,
            final SourceSetExtension.SourceSetConfiguration mainSourceSet) {
		...
		run.getArguments().addAll(
				extension.getSplitGenerationOutputs().flatMap(split -> projectExtension.getModId().map(modId -> {
					...
					dataRunArguments.add(output.getAbsolutePath());
